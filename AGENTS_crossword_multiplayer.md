...
- Статус: `lobby`. Кнопки карточки: **«Пригласить / Создать ссылку / Старт»** (первый пункт открывает стандартный выбор контактов). Карточка лобби (динамическое сообщение) создаётся через `_publish_lobby_message`, а при изменении состава/статуса обновляется `_update_lobby_message` с актуальной клавиатурой и подписями.
...
- Deep‑link и join‑коды выдаются `_assign_join_code`: по нажатию «Создать ссылку» `lobby_link_callback_handler` возвращает кнопку с `t.me/<bot>?start=join_<code>` и текстовым кодом для ручного ввода.
