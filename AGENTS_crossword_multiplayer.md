# Telegram Bot «Кроссворды» — мультиплеер (пошаговый режим)
...
- **Хаб** — групповой чат «комнаты» игры: бот публикует сетку, подсказки, чей ход, очки, таймер. 
- **Спицы** — личные чаты игроков: бот может продублировать приватные уведомления (например, предупреждение о времени, персональные подсказки). Механика приглашений и фиксация player‑chat ID берутся из паттерна другого бота. 
...
- Статус: `lobby`. Кнопки: **«Пригласить из контактов»**, **«Создать ссылку»**, **«Старт»**. Приглашения берутся из второго бота (контакты + deep‑link `t.me/<bot>?start=join_<code>`). 
...
3) **Старт игры**
- Бот объявляет режим: **пошагово**. Создаёт `turn_order` (случайно или по порядку входа). Выставляет **ход игрока №1** и публикует в группе: «Ход @username — выберите ячейку».
- Таймер игры опционально: фиксированная длительность (напр., 10 минут) с предупреждением **за 1 минуту** (job queue). Лимит на ход (например, 30 сек) — отдельный таймер; при истечении — `пропуск хода`. 
...
- Дамми‑игрок имитирует человека:
  ...
  - **Задержка ответа**: `DUMMY_DELAY_RANGE=(3..8)` секунд, случайно. Планируется через JobQueue сразу после объявления хода дамми.
...
- `Config`: `DUMMY_ACCURACY=0.8`, `DUMMY_DELAY_RANGE=(3,8)`, `ADMIN_FIRST=False`.
