...
- Если ответ верный — заполняем клетки, обновляем PNG, увеличиваем очки игрока (сложность/длина слота могут давать разный вес), объявляем «Верно!» и показываем обновлённую сетку. Если неверно — повторно публикуются подсказки и обновлённая сетка с отметкой о неверном ходе (без отдельного текстового уведомления), после чего ход переходит к следующему игроку (включая тестовый режим).
